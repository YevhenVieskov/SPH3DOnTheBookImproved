...
c     load initial particle information from external disk file
	...
	if(breakdam) call bdam(x, vx, mass, rho, p, u, 
     &                        itype, hsml, ntotal)

	if(drop ) call circle(x, vx, mass, rho, p, u, 
     &                        itype, hsml, ntotal)

	if(itank) call tank(x, vx, mass, rho, p, u, 
     &                        itype, hsml, ntotal)
...
c     Giving mass and smoothing length as well as other data.
	...
	subroutine circle(x, vx, mass, rho, p, u, 
     &                        itype, hsml, ntotal)
...
c     ntotal-- total particle number                               [out]
     ...
     &     rho(maxn), p(maxn), u(maxn), hsml(maxn)
      integer i, j, d, m, n, mp, np, k
      double precision xl, yl, dx, dy,r,xtemp(dim, maxn),cr
...
!      ntotal = mp * np
	...
	do j = 1, np
	  k = j + (i-1)*np
	  xtemp(1, k) = (i-1)*dx + dx/2.
	  xtemp(2, k) = (j-1)*dy + dy/2. 
	 
        enddo
      enddo

	do k=1,mp*np
	cr=(xtemp(1,k)-0.5)*(xtemp(1,k)-0.5)+
     &	(xtemp(2,k)-0.5)*(xtemp(2,k)-0.5)
	  if(cr<=r*r) then
	    ntotal=ntotal+1
