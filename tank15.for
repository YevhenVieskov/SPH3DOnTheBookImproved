      subroutine tank15(x, vx, mass, rho, p, u, 
     &                        itype, hsml, ntotal)
...
c     ntotal-- total particle number                               [out]
     ...
     &     rho(maxn), p(maxn), u(maxn), hsml(maxn)
      ...
      double precision xl, yl, dx, dy,c(maxn)
	...
	double precision  dx2,dy2,xtemp(dim, maxn)
...
c     &		1111)
	...
	subroutine dbp_part_tank15(itimestep, ntotal,nghost,hsml,mass,x,vx,
     &           rho,u,p,itype)
	 use ex
	implicit none
      include 'param.inc'
	...
	double precision dxiac(maxn),dr,dx,dy,xl,v_inf,dr1,dr2,xl2
	double precision XMleft,XMright,YMbottom,YMtop,XBleft,XBright
      double precision YBbottom,YBtop,XPleft,XPright,YPbottom,YPtop 
