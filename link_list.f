...
c     3-dim. problem: maxngx = maxngy = maxngz ~ maxn^(1/3)
     ...
     &       dwdx(:,:)
      ...
      integer grid(maxngx,maxngy,maxngz),gcell(3),
     &     xcell,ycell,zcell,minxcell(3),maxxcell(3)
     ...
     &tdwdx(:), dgeomx(:)
      allocate (xgcell(3,maxn), celldata(maxn),dnxgcell(dim),
     &    dpxgcell(dim),ngridx(dim),ghsmlx(dim),dx(dim),mingridx(dim),
     &    maxgridx(dim),  tdwdx(dim), dgeomx(dim))
      if (skf.eq.1) then 
        scale_k = 2 
...
c     Position particles on grid and create linked list:
      ...
      do i=1,ntotal
        call grid_geom(i,x(1:dim,i),ngridx,maxgridx,
     &                 mingridx,dgeomx,gcell) !!!!!!!!!!!!!!!!!!
        do d=1,dim
          xgcell(d,i) = gcell(d)
        enddo
...
c     Statistics for the interaction
	...
	if (countiac(i).lt.miniac) then 
      ...
      deallocate (xgcell, celldata,dnxgcell,
     &    dpxgcell,ngridx,ghsmlx,dx,mingridx,
     &    maxgridx,  tdwdx, dgeomx)
      end
